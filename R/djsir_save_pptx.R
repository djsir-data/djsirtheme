#' Export a ggplot to a DJSIR SPP PowerPoint slide.
#'
...
#' @param plot The ggplot to turn into a slide. Default is `ggplot2::last_plot()`.
#'
...
#' djsir_save_pptx("output.pptx", the_ggplot)
#'
#' # Specify layout and signpost
#' djsir_save_pptx("output.pptx", the_ggplot, layout = "half", signpost = "section")
#'
#' # For slide layouts other than 'full', you can add text commentary in the
#' # sidebar of the slide, like this:
#'
#' the_ggplot_with_sidebar <- the_ggplot +
#'   labs(sidebar = c("These are some comments", "For the sidebar of the slide"))
#'
#' djsir_save_pptx("output.pptx", the_ggplot_with_sidebar, layout = "twothirds")
#'
#' # Export multiple slides. Different plots can be passed in for each slide.
#' djsir_save_pptx(NULL, the_ggplot) %>%
#'   djsir_save_pptx(the_ggplot, layout="half") %>%
#'   print(target="slidepack.pptx")
#' }
djsir_save_pptx <- function(destination = NULL,
                            plot = ggplot2::last_plot(),
                            layout = c("full", "half", "twothirds"),
                            signpost = NULL) {
  UseMethod("djsir_save_pptx")
}

#' @export
djsir_save_pptx.NULL <- function(destination, ...) {
  djsir_save_pptx(get_template(), ...)
}

#' @export
djsir_save_pptx.character <- function(destination, ...) {
  slide <- djsir_save_pptx(NULL, ...)
  print(slide, target = destination)
}

#' @export
djsir_save_pptx.rpptx <- function(destination,
                                  plot = ggplot2::last_plot(),
                                  layout = c("full", "half", "twothirds"),
                                  signpost = NULL) {
...
get_template <- function() {
  ...
  path <- system.file("extdata", "template.pptx", package = "djsirtheme")
